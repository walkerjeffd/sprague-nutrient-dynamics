...
# load stn ----
stn.usgs <- read.table(file.path(DATA_DIR, 'sprague', 'usgs', 'usgs_stations.rdb'), 
                       sep='\t',
                       skip=77,
  ...
  mutate(STATION_ID=factor(STATION_ID),
         SITE_NAME=plyr::revalue(STATION_ID,
                               c('11495800'='NF',
                                 '11497500'='Beatty',
                                 '11497550'='Godowa',
                                 '11501000'='Power')))
...
# load data ----
q.11495800 <- read.table(file=file.path(DATA_DIR, 'sprague', 'usgs', 
                                        '11495800_sprague_nf_1993-05-01_2012-10-10.txt'), 
                         skip=27, col.names=c('AGENCY', 'STATION_ID', 'DATE', 'FLOW', 'FLAG'), stringsAsFactors=FALSE)
q.11497500 <- read.table(file=file.path(DATA_DIR, 'sprague', 'usgs', 
                                        '11497500_sprague_beatty_1953-10-01_1991-09-30.txt'), 
                         skip=27, col.names=c('AGENCY', 'STATION_ID', 'DATE', 'FLOW', 'FLAG'), stringsAsFactors=FALSE)
q.11501000 <- read.table(file=file.path(DATA_DIR, 'sprague', 'usgs', 
...
q.usgs <- rbind(q.11501000, q.11495800, q.11497500)
...
q.usgs <- left_join(q.usgs, select(stn.usgs, STATION_ID, SITE_NAME), by='STATION_ID')
...
select(stn.usgs, SITE_NAME, STATION_ID, DESCRIPTION, LAT, LON, 
  ...
  write.csv(file=file.path('csv', 'stn_usgs.csv'), row.names=FALSE)

save(q.usgs, stn.usgs, file='usgs.Rdata')

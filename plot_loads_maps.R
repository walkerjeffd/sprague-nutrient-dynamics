...
tile_incbasin <- function(dataset, variable, term, title=NULL) {
  p <- filter(df_wyr, DATASET==dataset, VAR==variable, TERM==term, SITE_NAME %in% incbasin_levels[[dataset]]) %>%
    mutate(SITE_NAME=ordered(as.character(SITE_NAME), levels=rev(levels(SITE_NAME)))) %>%
    ggplot(aes(x=factor(WYEAR), y=SITE_NAME)) +
    geom_tile(aes(fill=VALUE)) +
    scale_fill_term[[term]] +
...
for (dataset in c('POR', 'RECENT')) {
  ...
  for (variable in variables) {
    pdf(file.path('pdf', tolower(dataset), 'loads-maps-subbasin', paste0('loads-subbasin-', tolower(variable), '.pdf')), width=11, height=8.5)
    cat('....', variable, '\n')
...
#                   paste0('Mean Annual Flow', '   |   ', 'Dataset: ', dataset, '   |   Variable: Flow\n'))
  ...
  cat('.. INCBASINS', '\n')
  for (variable in variables) {
    pdf(file.path('pdf', tolower(dataset), 'loads-maps-incremental-basin', paste0('loads-incbasin-', tolower(variable), '.pdf')), width=11, height=8.5)
    cat('....', variable, '\n')
...
#             strip.background=element_blank())
  ...
  pdf(file.path('pdf', tolower(dataset), paste0('loads-summary-maps.pdf')), width=10, height=8)
  grid.arrange(grobs=maps.c, nrow=2, main=paste0('\nNet Change in FWM Concentration by Incremental Subbasin\nDataset: ', dataset))
  makeFootnote('Map tiles by Stamen Design, under CC BY 3.0. Data by OpenStreetMap, under CC BY SA.')
  grid.arrange(grobs=maps.l, nrow=2, main=paste0('\nNet Load per Unit Area by Incremental Subbasin\nDataset: ', dataset))
  makeFootnote('Map tiles by Stamen Design, under CC BY 3.0. Data by OpenStreetMap, under CC BY SA.')
  grid.arrange(grobs=maps.q, nrow=2, ncol=3, main=paste0('\nNet Flow per Unit Area by Incremental Subbasin\nDataset: ', dataset))
  makeFootnote('Map tiles by Stamen Design, under CC BY 3.0. Data by OpenStreetMap, under CC BY SA.')
  dev.off()
}
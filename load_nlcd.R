...
nlcd <- rbind.fill(nlcd_basin, nlcd_valley) %>%
  dplyr::select(-X__fid__) %>%
  gather(LULC, COUNT, -SITE, -AreaSqKM, -EXTENT) %>%
  mutate(LULC=as.integer(parse_number(LULC)),
...
dplyr::group_by(nlcd.grp, EXTENT, INC_SITE_NAME, SUM_AREA_KM2) %>%
...
nlcd.cat <- nlcd %>%
  ...
  dplyr::group_by(SOURCE, EXTENT, INC_SITE_NAME, SUM_AREA_KM2, LANDUSE) %>%
  dplyr::summarise(AREA_KM2=sum(AREA_KM2)) %>% # this was creating a single value when we needed sums by group, fixed it by specifying package, it was pulling  a function from a different package
  ungroup %>%
  mutate(AREA_FRAC=ifelse(SUM_AREA_KM2>0, AREA_KM2/SUM_AREA_KM2, 0))
  dplyr::group_by(nlcd.cat, EXTENT, INC_SITE_NAME, SUM_AREA_KM2) %>%
  dplyr::summarise(AREA_FRAC=sum(AREA_FRAC))
...
filter(nlcd, EXTENT=='basin') %>%
  dplyr::group_by(SOURCE, INC_SITE_NAME) %>%
  dplyr::summarise(NLCD_AREA_KM2=sum(AREA_KM2)) %>%
  ungroup %>%
...
nlcd %>%
  dplyr::group_by(SOURCE, EXTENT, INC_SITE_NAME) %>%
  dplyr::summarise(SUM=sum(AREA_FRAC)) %>%
  filter(SUM != 0) %>%
  mutate(sum_all = sum(SUM))
  #(function(x) { stopifnot(all(x$SUM > 0.99999)) }) # check if all sums are greater than 0.9999? if not stop?

# summary of basin/valley areas
dplyr::group_by(nlcd, SOURCE, EXTENT, INC_SITE_NAME) %>%
...
nlcd.subbasin <- filter(nlcd, !(INC_SITE_NAME %in% "Godowa-SF-NF")) %>%
  ...
  dplyr::rename(SITE_NAME=INC_SITE_NAME) %>%
...
dplyr::group_by(nlcd.subbasin, SOURCE, EXTENT, SITE_NAME) %>%
  dplyr::summarise(AREA_FRAC=sum(AREA_FRAC)) %>%
  filter(AREA_FRAC != 0) %>%
  mutate(sum_all = sum(AREA_FRAC)) # this verifies that AREA_FRAC = 1
 # (function(x) { stopifnot(all(x$AREA_FRAC > 0.99999)) }) # troubleshoot this

...
filename <- file.path('pdf', 'dataset', 'dataset-qaqc-timeseries.pdf')
...
p <- filter(wq.kt_sprague$RAW, VAR %in% detection_limits$VAR) %>%
  ...
  scale_size_manual('QAQC', values=c('BLANK'=1.5, 'LOW'=1.5, 'PASS'=0.5, 'RPD'=1, 'OUTLIER'=1.5)) +
  scale_linetype_manual("Detection Limit", values=c("DL"=1, "HALF_DL"=2), labels=c("DL"="DL", "HALF_DL"="1/2 DL")) +
  labs(x='', y='Concentration (ppm)', title="QAQC Review of Water Quality Data") +

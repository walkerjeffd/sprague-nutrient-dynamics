...
#   }
  ...
  grid.arrange(p.4, arrangeGrob(p.2, ncol=2, widths=c(2/3, 1/3)), nrow=3, main=paste0('\n', title))
}
...
plot_diagnostic <- function(dataset, site_name, variable, term, log_trans=TRUE) {
  x.trend <- filter(trends, DATASET==dataset, SITE_NAME==site_name, VAR==variable, TERM==term, LOG==log_trans)
  ...
  x.mon <- filter(df_mon, DATASET==dataset, SITE_NAME==site_name, VAR==variable) %>%
    mutate(SEASON=ifelse(MONTH %in% seq(4, 9),
                         'Apr-Sep', 'Oct-Mar'),
...
#     guides(linetype=guide_legend(keywidth = 3, keyheight = 1)) +
  ...
  p.mon <- x.mon %>%
    ...
    geom_abline(aes(intercept=INTERCEPT, slope=SLOPE), 
                data=filter(x.mon.trends, SEASON %in% seq(1, 12)) %>% rename(MONTH=SEASON)) +
  ...
  grid.arrange(arrangeGrob(p.mon.ts, p.wyr.ts, widths=c(2/3, 1/3), nrow=1),
               p.mon,
               arrangeGrob(p.bar, tblGrb, widths=c(2/3, 1/3), nrow=1, just=c('left', 'top')),
               ncol=1,
               just=c('left', 'top'),
...
for (dataset in dataset_levels) {
  ...
  for (variable in dataset_vars[[dataset]]) {
    cat('..', variable, '\n')
    pdf(file.path('pdf', tolower(dataset), 'trends', paste0('trends-', tolower(variable), '.pdf')), width=11, height=8.5)
  ...
  cat('..', 'FLOW', '\n')
  pdf(file.path('pdf', tolower(dataset), 'trends', paste0('trends-', 'flow', '.pdf')), width=11, height=8.5)
...
for (dataset in dataset_levels) {
  cat(dataset, '\n')
  pdf(file.path('pdf', tolower(dataset), paste0('trends-summary.pdf')), width=11, height=8.5)
